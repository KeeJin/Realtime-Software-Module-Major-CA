               ...
               int beeper)  // sine wave function
{
  ...
  while ((wave_type == 0) &&
         (switch3_value(dio_switch)))  // stops if wave_type is not 1 (sine)
  {
    for (i = 0; i < N; i++) {
      if (!(wave_type == 0) || !(switch3_value(dio_switch))) return;
#if PCI
      data = (((sinf((float)(i * 2 * 3.1415 / N)) * amplitude) + 10 +
               vert_offset) /
...
{
  while ((wave_type == 1) &&
         (switch3_value(dio_switch)))  // stops if wave_type is not 2 (square)
  {
    if (beeper)
...
void triangular_wave(uintptr_t dio_switch, WaveType wave_type, float amplitude,
  ...
  while (
      (wave_type == 2) &&
      (switch3_value(dio_switch)))  // stops if wave_type is not 3 (triangular)
  {
    for (i = 0; i < N / 2; i++) {
      if (!(wave_type == 2) || !(switch3_value(dio_switch))) return;
#if PCI
      data = ((vert_offset - amplitude + amplitude * 4 * i / N + 10) / 20.0 *
              0xFFFF);
...
#if PCI
      data = ((vert_offset - amplitude + amplitude * 2 * i / N + 10) / 20.0 *
              0xFFFF);
      out16(DA_CTLREG, 0x0923);  // DA Enable, #0, #1, SW 10V bipolar
      out16(DA_FIFOCLR, 0);      // Clear DA FIFO  buffer
      out16(DA_Data, (short)data);
...
void zero_signal(uintptr_t dio_switch, WaveType wave_type, float amplitude,
  ...
  while ((wave_type == 4) ||
         !(switch3_value(
             dio_switch)))  // stops if wave_type is not 0 (zero voltage)
  {
    for (i = 0; i < N; i++) {
